...
from core.dataset import MBSpaceDataBase
from core.dataset import MBSpaceDataset, CachedSubset

from core.model import MBSpaceNAC
...
if __name__ == "__main__":
    ...
    nac = MBSpaceNAC(20*3*2, 512).to(device=device)

    max_epochs = args.nac_epochs

    if args.space == "mobilespace":
        database = MBSpaceDataBase.from_file(args.data)
        dataset = MBSpaceDataset(database=database, seed=args.seed)
    ...
    # train controller
    controller = MBSpaceController(device=device).to(device=device)
    ...
    train_controller(max_iter=args.pl_iters, entropy_coeff=args.entropy_coeff, grad_clip=args.controller_grad_clip,
                     ...
                     alternate_train=alternate_train, alternate_evaluate=alternate_evaluate,
